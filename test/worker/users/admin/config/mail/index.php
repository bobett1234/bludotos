...
for (x in thisis)
{
	...
	ajax3.onreadystatechange = function() {
		if (ajax3.readyState==4) {
                  ...
                  Doc.ajax = function (url, callback, type, send) {  
        var xhr;  
	  ...
	  if(html.length > 0){
        ...
        if(!send){
        xhr.open('GET', 'users/<? echo $user; ?>/sysapps/FileNet/HDD/Applications/temp/<? echo $name; ?>/'+url, true);  
        } else {
        xhr.open('GET', 'users/<? echo $user; ?>/sysapps/FileNet/HDD/Applications/temp/<? echo $name; ?>/'+url+'?'+send, true); 
        } 
        xhr.send('');  
    }
